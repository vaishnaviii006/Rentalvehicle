...
// Fallback Offline Dummy Data in case server is booting/offline
...
export default function App() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      console.log('Backend connection failed. Using local storage/offline fallbacks.');
  ...
  const handlePickup = async (bookingId, pickupData) => {
    if (backendActive) {
      try {
        const res = await fetch(`http://localhost:5000/api/bookings/${bookingId}/pickup`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleExtend = async (bookingId, extendData) => {
    if (backendActive) {
      try {
        const res = await fetch(`http://localhost:5000/api/bookings/${bookingId}/extend`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleReplaceVehicle = async (bookingId, replaceData) => {
    if (backendActive) {
      try {
        const res = await fetch(`http://localhost:5000/api/bookings/${bookingId}/replace`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleDropOff = async (bookingId, dropOffData) => {
    if (backendActive) {
      try {
        const res = await fetch(`http://localhost:5000/api/bookings/${bookingId}/dropoff`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleAdminOverride = async (bookingId, overrideData) => {
    if (backendActive) {
      try {
        const res = await fetch(`http://localhost:5000/api/bookings/${bookingId}/override`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
  ...
  // Local helper for Settlement handover logging in offline mode
  const handleRecordDeposit = (date, workerId, amount, remarks) => {
    // If offline, we just alert or locally manage it. 
    // Since settlement fetches dynamically, the component calculates worker balances locally.
    console.log(`Local Cash settlement handover recorded: ₹${amount} deposited from worker ${workerId}.`);
  };
  ...
  const renderTabContent = () => {
    ...
    switch (currentTab) {
      case 'dashboard':
        return (
          <DashboardHome 
            ...
            startPickup={(b) => { setSelectedBookingForOps(b, 'pickup'); }}
            startDropOff={(b) => { setSelectedBookingForOps(b, 'dropoff'); }}
          />
        );
      case 'available':
        return (
      ...
      default:
        return <DashboardHome vehicles={vehicles} bookings={bookings} userRole={userRole} />;
    }
  };
